...
@section('title', 'Nueva Imagen/Video')
@section('page-title', 'Agregar Nueva Imagen o Video a la Galería')
        ...
        <form action="{{ route('admin.gallery.store') }}" method="POST" enctype="multipart/form-data" id="galleryForm">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <div class="flex gap-4">
                        <label class="flex items-center cursor-pointer">
                            <input type="radio" name="type" value="image" {{ old('type', 'image') == 'image' ? 'checked' : '' }} class="mr-2" onchange="updateFileAccept()">
                            <span class="text-gray-700">Imagen</span>
                        </label>
                        <label class="flex items-center cursor-pointer">
                            <input type="radio" name="type" value="video" {{ old('type') == 'video' ? 'checked' : '' }} class="mr-2" onchange="updateFileAccept()">
                ...
                <div>
                    ...
                    <input type="file" name="image" id="fileInput" accept="image/jpeg,image/png,image/jpg,image/webp" required
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('image') border-red-500 @enderror">
                    <p class="text-xs text-gray-500 mt-1" id="fileHint">
                        <span class="imageHint">Formatos: JPG, PNG, WEBP. Máx: 5MB</span>
                        <span class="videoHint hidden">Formatos: MP4, MOV, AVI, WMV. Máx: 100MB</span>
                    </p>
                    @error('image')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
